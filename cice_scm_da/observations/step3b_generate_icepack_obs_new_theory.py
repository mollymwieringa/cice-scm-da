...
# Set reference ensemble case
case = 'free_SHEBA'
dart_dir = '/glade/work/mollyw/dart_manhattan/'

# Set observation network file
observation_file = '/glade/work/mollyw/Icepack_DART/files/icepack_observations.nc'
observation_store = '/glade/work/mollyw/DA_obs/ICEPACK_OBS/'+case+'/eakf_vs_brhf/mem_14/bc_obs/'
# observation_store = '/glade/work/mollyw/DA_obs/TEST/'+case+'/'
if category is True:
    observation_store = observation_store + 'itd/'

if error_type == 'scaled':
    observation_store = observation_store + 'scaled/'

# Set truth member file
rand_member = 14
...
observation_data = xr.open_dataset(observation_file)
observation_times = observation_data.year_mon_day.values
...
for t in range(0, datelist.shape[0]):
    ...
    truth_file = '/glade/scratch/mollyw/ICEPACK_RUNS/'+case+'/mem00{00}/restart/'.format('%02d'%rand_member)+'iced.{0}'.format('%04d'%int(year))+'-{0}'.format('%02d'%int(mon))+'-{0}'.format('%02d'%int(day))+'-00000.nc'
    
    # I THINK THIS IS WHERE THE REAL OBSERVATION FILE SHOULD GO 
    # os.symlink(dart_dir + '/models/cice-scm2/work/cice.r.nc', 'cice.r.nc')
    os.symlink(truth_file, 'cice.r.nc')
    
    if category is True:
        null = unpack_categories(truth_file)
        null = unpack_categories(dart_dir + '/models/cice-scm2/work/cice.r.nc')
    ...
    lat = observation_data.latitude.values[0]
    lon = observation_data.longitude.values[0]
    ...
    inpt_status = write_blank_obs_seq(lat, lon, year, mon, day, obs_seq_in, ob_types, error = error_type, case_name=case)
    if inpt_status < 0:
    ...
    comd = dart_dir + '/models/cice-scm2/work/create_obs_sequence < input.txt > output.create_obs_sequence'
    os.system(comd)
    ...
    comd = dart_dir + '/models/cice-scm2/work/perfect_model_obs > output.perfect_model_obs_'+str(yesterday)+'_'+hour_tag
    os.system(comd)
    ...
    # comd = 'ls obs_seq.out > obs_seq_flist'
    # os.system(comd)
    # comd = '/glade/work/mollyw/Icepack_DART/DART/models/cice-scm/work/create_obs_sequence/obs_sequence_tool > output.obs_sequence_tool'
    # os.system(comd)
    # ost_status = check_completion('output.obs_sequence_tool')
    # if ost_status < 0:
    #     print('obs_sequence_tool did not complete. Process stopping.')
    #     sys.exit()
    ...
    if os.path.exists('obs_seq.'+str(today)) is False:
        ...
        if os.path.exists(observation_store + '/raw_files/') is False:
            os.makedirs(observation_store + '/raw_files/')
        shutil.move('obs_seq.in_'+str(yesterday)+'_00000', observation_store+'/raw_files/') 
...
# Move all generated observation sequence files to observation_store
comd = 'mv obs_seq.* '+ observation_store
os.system(comd)
num_files = len(glob.glob(observation_store + '/obs_seq.*'))
