...
source_case = sys.argv[2] # 'free_test'
...
obs_dir = project_dir + '/data/processed/synthetic_obs/'+source_case
# where is the scratch directory for this user?
scratch_dir = '/glade/scratch/mollyw/'

# set the assimilation parameters
filter_kind = int(sys.argv[3]) #1
regression_kind = sys.argv[4] #'NORM'
postprocessing = 'cice'
...
def cycle_da(case, source_case, dart_dir, filter_kind, assim_date, truth_member, cat_obs = False):
    ...
    # Set up directory
    ensemble_size = len(glob.glob('/glade/scratch/mollyw/ICEPACK_RUNS/'+source_case+'/mem*'))
    mem = 1
...
for date in datelist:
    cycle_da(case, source_case, dart_dir, filter_kind, date, truth_member, cat_obs=cat_assim)
