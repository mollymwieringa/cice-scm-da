...
if sys.argv[2] == 'all':
    ...
    stages = [sys.argv[i] for i in range(2,len(sys.argv))]
...
for stage in stages:
    if stage in ['input','preassim','analysis','output']:
        ...
        base_path = '/glade/scratch/'+user+'/ICEPACK_RUNS/'+case+'/analyses/'
    elif stage in'forecast':
        base_path = '/glade/scratch/'+user+'/ICEPACK_RUNS/'+case+'/forecasts/'
    else:
